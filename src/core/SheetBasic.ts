import { Cell, SheetData, SheetInternalState, Rect } from 'types';
...
export class SheetStateWatcher extends EventEmmit<
  SheetEventNames,
  SheetOperations,
  ...
  dispatch(
    ...
    switch (action.type) {
      ...
      case 'UpdateCells': {
        ...
        if (!this.state.matrix[y]) {
          this.state.matrix[y] = {};
        }
        if (!value) {
          delete this.state.matrix[y][x];
        } else {
...
class SheetBasic extends SheetStateWatcher {
...
export class SheetUtils {
  ...
  isCursorOnSerieBox(offsetX: number, offsetY: number) {
    ...
    const pos = this.distanceToStartIndex(x, y);

    if (pos) {
      const size = DRAGGER_SIZE;
      const draggerXStart = pos[0] + state.cols[-1] + width - size / 2;
      const draggerYStart = pos[1] + state.rows[-1] + height - size / 2;
  ...
  *createCellsIterator({
  ...
  distanceToStartIndex(x: number, y: number, onlyInView = true) {
    const { state } = this;

    // we treat the startIndex as origin
    const [colStartIdx, rowStartIdx] = state.startIndexs;
    const { width, height } = this.injection.getConfig();
    const xOffsetBound = onlyInView ? width : Number.MAX_SAFE_INTEGER;
    const yOffsetBound = onlyInView ? height : Number.MAX_SAFE_INTEGER;
  ...
  distanceToCanvasOrigin(x: number, y: number, onlyInView = true) {
  ...
  mouseOffsetToGridIndex(offsetX: number, offsetY: number) {
    const { state } = this;
